...
const CommunityCommentChatCard = ({ data, hideComment }) => {
    ...
    const { id } = useParams();
    const { isMyPost, isAnonymouse, _id, selfLiked, selfDisliked, authorRole, content, authorDetails, image, viewsCount, selfViews, likeCount, dislikeCount, commentCount, isShowComment } = postDetails;

    const likePost = (id) => {
        if (selfLiked) {
            return;
        }
        authAxios.get(`/community/post/${id}/like`).then((response) => {
            setPostDetails((prevDetails) => ({
                ...prevDetails,
                likeCount: prevDetails.likeCount + 1,
                selfLiked: true,
                selfDisliked: false,
                dislikeCount: selfDisliked ? Math.max(prevDetails.dislikeCount - 1, 0) : prevDetails.dislikeCount,
            }));
    ...
    const dislikePost = (id) => {
        ...
        authAxios.get(`/community/post/${id}/dislike`).then((response) => {
            setPostDetails((prevDetails) => ({
                ...prevDetails,
                dislikeCount: prevDetails.dislikeCount + 1,
                selfDisliked: true,
                selfLiked: false,
                likeCount: selfLiked ? Math.max(prevDetails.likeCount - 1, 0) : prevDetails.likeCount,
            }));
    ...
    const markFlag = (flag, postId, communityId) => {
        authAxios.post(`/community/flag/add`,
            {
    ...
    return (
        <div className='CommunityCommetBody'>
            <div className='aritcleUseImg'>
                {isAnonymouse ? (
                    <img className='img-fluid' src='/assets/images/default_user.jpg' alt='Anonymous User' />
                ) : (
                    <img className='img-fluid' src={authorDetails?.profileImage?.path || '/assets/images/default_user.jpg'} alt={authorDetails?.profileImage?.path || 'Patient Image'} />
                )}
            </div>
            <div className='aritcleCommentContent'>
                {isAnonymouse ? (
                    ...
                    <Fragment>
                        {authorRole === 'doctor' && (
                            <h4>Dr. {authorDetails?.firstName + ' ' + authorDetails?.lastName}</h4>
                        )}
                        {authorRole === 'patient' && (
                            <h4>{authorDetails?.fullName}</h4>
                        )}
                        {authorRole === 'admin' && (
                            <h4>Admin</h4>
                        )}
                ...
                {image?.path && (
                    <div className='chatImg mb-2'>
                        <img className='img-fluid' src={image?.path || '/assets/images/default_img.jpg'} alt={image?.path || 'Patient Image'} />
                    </div>
                )}
                <ul className='communityReplyComtLts'>
                    ...
                    <li>
                        <span onClick={() => likePost(_id)} style={{ color: selfLiked ? '#019889' : 'inherit' }}>
                            ...
                            <Dropdown className="flagDropdown">
                                <Dropdown.Toggle variant="none" id="dropdown-basic">
                                    <svg width="12" height="14" viewBox="0 0 10 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1.2998 7V10.75C1.2998 10.9271 1.2398 11.0756 1.1198 11.1956C0.999805 11.3156 0.851471 11.3754 0.674805 11.375C0.498138 11.3746 0.349805 11.3146 0.229805 11.195C0.109805 11.0754 0.0498047 10.9271 0.0498047 10.75V1.375C0.0498047 1.19792 0.109805 1.04958 0.229805 0.93C0.349805 0.810417 0.498138 0.750417 0.674805 0.75H5.15918C5.30501 0.75 5.43522 0.796875 5.5498 0.890625C5.66439 0.984375 5.7373 1.10417 5.76855 1.25L5.9248 2H8.7998C8.97689 2 9.12543 2.06 9.24543 2.18C9.36543 2.3 9.42522 2.44833 9.4248 2.625V7.625C9.4248 7.80208 9.3648 7.95063 9.2448 8.07063C9.1248 8.19063 8.97647 8.25042 8.7998 8.25H5.56543C5.4196 8.25 5.28939 8.20312 5.1748 8.10938C5.06022 8.01562 4.9873 7.89583 4.95605 7.75L4.7998 7H1.2998ZM6.08105 7H8.1748V3.25H5.40918C5.26335 3.25 5.13314 3.20312 5.01855 3.10938C4.90397 3.01562 4.83105 2.89583 4.7998 2.75L4.64355 2H1.2998V5.75H5.31543C5.46126 5.75 5.59147 5.79688 5.70605 5.89062C5.82064 5.98438 5.89355 6.10417 5.9248 6.25L6.08105 7Z" fill="#019889" />
                                    </svg>
                                </Dropdown.Toggle>

                                <Dropdown.Menu>
                                    <Dropdown.Item onClick={() => markFlag("Mark as inappropriate", _id, id)}>Mark as inappropriate</Dropdown.Item>
                                    <Dropdown.Item onClick={() => markFlag("Mark as Abuse", _id, id)}>Mark as Abuse</Dropdown.Item>
                                </Dropdown.Menu>
                            </Dropdown>
                        </li>
                    )}
