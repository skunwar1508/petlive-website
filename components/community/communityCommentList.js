...
const CommunitypostList = ({ id }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (newpost.trim() || postImage?._id) {
            ...
            authAxios.post(`/community/post/${id}`, {
                content: newpost,
    ...
    const postViews = (ids) => {
        authAxios.post(`/community/postviews`, ids).then((response) => {
            setPosts((prevposts) => {
    ...
    return (
        <div className="communityChatBody">
            ...
            >
                {posts.map((post) => (
                    <CommunitypostChatCard key={post?._id} data={post} hideComment={(id)=>hideComment(id)} />
                ))}
            </div>
